        ...
        <div class="bg-white dark:bg-neutral-800 shadow-xl sm:rounded-lg">
            <div class="p-6">
                ...
                <form wire:submit.prevent="searchCitizen" class="space-y-4">
                    <div>
                        <label for="search_travel" class="block text-sm font-medium leading-6 text-gray-900 dark:text-neutral-200">{{ __('Nome, CPF ou CNS do Cidadão') }}</label>
                        <div class="mt-2">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 pt-2">
                        <a href="{{ route('travel-requests.index') }}" wire:navigate {{-- Rota para cancelar e voltar ao index de solicitações de viagem --}}
                ...
                {{-- Exibe a mensagem de info diretamente da propriedade do componente --}}
                @if($infoMessage)
                    <div class="mt-4 p-3 text-sm bg-yellow-100 dark:bg-yellow-700/30 text-yellow-700 dark:text-yellow-300 rounded-md">
                        {{ $infoMessage }}
                    </div>
                @endif
                {{-- Ou use o seu partial de mensagens se preferir lidar com session flashes --}}
                            ...
                            @foreach ($results as $citizen)
                                <li wire:key="citizen-result-{{$citizen->id}}" class="border dark:border-neutral-700 p-4 rounded-md shadow-sm text-sm text-gray-700 dark:text-neutral-300 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 hover:bg-gray-50 dark:hover:bg-neutral-700/50">
                                    <div class="flex-grow">
                                        ...
                                        <p><strong>{{__('Nascimento:')}}<p><strong>{{__('Nascimento:')}}</strong> {{ $citizen->date_of_birth ? \Carbon\Carbon::createFromFormat('d/m/Y', $citizen->date_of_birth)->format('d/m/Y') : 'N/A' }}</p>
                                    </div>
