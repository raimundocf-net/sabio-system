<div>
    <x-slot:title>
        {{ $pageTitle }} {{-- Definido como "Editar Usuário: Nome" no UpdateUser.php --}}
    </x-slot:title>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-neutral-800 shadow-md sm:rounded-lg">
            {{-- O método no componente UpdateUser.php é updateUser() --}}
            <form wire:submit.prevent="updateUser">
                <div class="p-6">
                    {{-- Inclui o formulário parcial, passando isEditing como true --}}
                    @include('livewire.users._form-user', ['isEditing' => true])
                </div>
                ...
                <div class="flex items-center justify-end space-x-3 bg-gray-50 dark:bg-neutral-800/50 px-6 py-4 border-t border-gray-200 dark:border-neutral-700 sm:rounded-b-lg">
                    ...
                    <button type="submit"
                        ...
                        <span wire:loading wire:target="updateUser">{{__('Salvando...')}}</span>
                        <span wire:loading.remove wire:target="updateUser">{{ __('Salvar Alterações') }}</span>
                    </button>
                </div>
            </form>
        </div>
