<div>
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-white dark:bg-neutral-800 shadow sm:rounded-lg">
            <div>
                <label for="searchTerm" class="block text-sm font-medium text-gray-700 dark:text-neutral-300">{{__('Buscar')}}</label>
                <input type="text" wire:model.live.debounce.300ms="searchTerm" id="searchTerm" placeholder="{{__('Nome ou Email')}}"
            ...
            <div>
                <label for="filterRole" class="block text-sm font-medium text-gray-700 dark:text-neutral-300">{{__('Papel')}}</label>
            ...
            <div>
                <label for="filterUnitId" class="block text-sm font-medium text-gray-700 dark:text-neutral-300">{{__('Unidade')}}</label>
                <select wire:model.live="filterUnitId" id="filterUnitId"
                        class="mt-1 block w-full rounded-md border-0 py-2 pl-3 pr-10 text-gray-900 dark:text-neutral-100 bg-white dark:bg-neutral-700 shadow-sm ring-1 ring-inset ring-gray-300 dark:ring-neutral-600 focus:ring-2 focus:ring-inset focus:ring-indigo-600 dark:focus:ring-sky-500 sm:text-sm">
                    <option value="">{{__('Todas as Unidades')}}</option>
        ...
        <div class="overflow-x-auto rounded-lg border border-neutral-200 dark:border-neutral-700">
            <table class="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700">
                ...
                <tr>
                    ...
                    <th class="relative px-6 py-3"><span class="sr-only">{{ __('Ações') }}</span></th>
                </tr>
                </thead>
                ...
                @forelse ($users as $user)
                    <tr wire:key="user-{{ $user->id }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                            <a href="{{ route('users.edit', $user) }}" wire:navigate class="inline-flex items-center text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300" title="{{ __('Editar') }}">
                                <span class="icon-[mdi--account-edit-outline] w-5 h-5"></span>
                            </a>
                            <button wire:click="openDeleteModal({{ $user->id }})" class="inline-flex items-center text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300" title="{{ __('Excluir') }}">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-4 text-center text-sm text-neutral-500 dark:text-neutral-400">{{ __('Nenhum usuário encontrado.') }}</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
        ...
        @if ($users->hasPages())
            <div class="px-2 py-2">{{ $users->links() }}</div>
        @endif
                        ...
                        <div class="bg-white dark:bg-neutral-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="sm:flex sm:items-start">
                                ...
                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-neutral-100" id="modal-title">{{ __('Confirmar Exclusão') }}</h3>
                                    <div class="mt-2">
                                        <p class="text-sm text-gray-600 dark:text-neutral-300">
                                            {{ __('Deseja excluir o usuário') }} <strong>{{ $deletingUser->name }}</strong>?
