<div>
    ...
    <div class="flex flex-col gap-4 mb-6 mt-4 sm:flex-row sm:items-center sm:justify-between px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center w-full sm:w-auto">
            ...
            <div class="sm:hidden">
                <a href="{{ route('companions.create') }}" wire:navigate
        ...
        <div class="w-full sm:flex-1 flex flex-col sm:flex-row flex-wrap gap-2 justify-start sm:justify-end items-center">
            ...
            <div class="w-full sm:w-auto flex-grow sm:flex-grow-0 sm:max-w-md">
                <input type="text" wire:model.live.debounce.300ms="searchTerm" placeholder="{{__('Buscar por Nome, CPF ou Documento...')}}"
            ...
            <div class="hidden sm:flex">
                <a href="{{ route('companions.create') }}" wire:navigate
                   class="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 text-sm font-medium bg-indigo-600 text-white hover:bg-indigo-700 dark:bg-sky-500 dark:hover:bg-sky-400 px-3 py-1.5 rounded-md shadow-sm">
                    <span class="icon-[mdi--account-plus] w-5 h-5"></span> {{ __('Novo Acompanhante') }}
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="px-4 sm:px-6 lg:px-8">
        <div class="overflow-x-auto shadow-md sm:rounded-lg border border-gray-200 dark:border-neutral-700">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
                ...
                <tr>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-neutral-300 uppercase tracking-wider">{{__('Nome Completo')}}</th>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-neutral-300 uppercase tracking-wider">{{__('CPF')}}</th>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-neutral-300 uppercase tracking-wider">{{__('Documento')}}</th>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-neutral-300 uppercase tracking-wider">{{__('Telefone')}}</th>
                ...
                @forelse($companions as $companion)
                    <tr wire:key="companion-row-{{ $companion->id }}" class="hover:bg-gray-50 dark:hover:bg-neutral-700/30">
                        <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-neutral-100">{{ $companion->full_name }}</td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700 dark:text-neutral-300">{{ $companion->cpf ?: 'N/D' }}</td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700 dark:text-neutral-300">{{ $companion->identity_document ?: 'N/D' }}</td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700 dark:text-neutral-300">{{ $companion->contact_phone ?: 'N/D' }}</td>
                        <td class="px-4 py-3 whitespace-nowrap text-right text-sm font-medium space-x-1">
                            <a href="{{ route('companions.edit', $companion->id) }}" wire:navigate title="{{__('Editar Acompanhante')}}"
                            ...
                            <button wire:click="openDeleteModal({{ $companion->id }})" title="{{__('Excluir Acompanhante')}}"
                    ...
                    <tr>
                        <td colspan="5" class="px-4 py-16 text-center text-sm text-gray-500 dark:text-neutral-400">
                            <div class="flex flex-col items-center">
                                ...
                                {{ __('Nenhum acompanhante cadastrado.') }}
                                <a href="{{ route('companions.create') }}" wire:navigate class="mt-2 text-sm text-indigo-600 hover:text-indigo-500 dark:text-sky-400 dark:hover:text-sky-300 font-medium">
                                    {{ __('Cadastrar Novo Acompanhante') }}
                                </a>
                            </div>
                        </td>
                    </tr>
        ...
        @if ($companions->hasPages())
            <div class="py-4 px-1">
                {{ $companions->links(data: ['scrollTo' => false]) }}
            </div>
        @endif
    </div>

    {{-- Modal de Exclusão --}}
    @if($showDeleteModal && $deletingCompanion)
            ...
            <div class="inline-block align-bottom bg-white dark:bg-neutral-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full pointer-events-auto">
                <div class="bg-white dark:bg-neutral-800 px-4 pt-5 pb-4 sm:p-6">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-neutral-100" id="modal-title-delete-companion">{{ __('Confirmar Exclusão') }}</h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-600 dark:text-neutral-300">
                                    {{ __('Você tem certeza que deseja excluir o acompanhante') }} <strong>"{{ $deletingCompanion->full_name }}"</strong>?
                                    {{ __('Esta ação não poderá ser desfeita (a menos que SoftDeletes esteja habilitado e haja uma função de restaurar).') }}
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-gray-50 dark:bg-neutral-800/50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-3">
                    <button wire:click="deleteCompanion" type="button" wire:loading.attr="disabled"
                        ...
                        <span wire:loading.remove wire:target="deleteCompanion">{{ __('Excluir') }}</span>
