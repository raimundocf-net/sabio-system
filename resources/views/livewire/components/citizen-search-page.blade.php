<div>
    ...
    <div class="max-w-2xl mx-auto mt-8">
        <div class="bg-white dark:bg-neutral-800 shadow-xl sm:rounded-lg">
            <div class="p-6">
                <h2 class="text-xl font-semibold text-gray-900 dark:text-neutral-100 mb-1">{{ $pageTitle }}</h2>
                ...
                <form wire:submit.prevent="searchCitizen" class="space-y-4">
                    <div>
                        <label for="generic_search" class="block text-sm font-medium leading-6 text-gray-900 dark:text-neutral-200">{{ __('Nome, CPF ou CNS do Cidadão') }}</label>
                        <div class="mt-2">
                            <input type="text" wire:model.defer="search" id="generic_search" placeholder="{{__('Digite Nome, CPF ou CNS')}}"
                    ...
                    <div>
                        <label for="generic_searchMother" class="block text-sm font-medium leading-6 text-gray-900 dark:text-neutral-200">{{ __('Nome da Mãe (Opcional)') }}</label>
                        <div class="mt-2">
                            <input type="text" wire:model.defer="searchMother" id="generic_searchMother" placeholder="{{__('Digite o nome da mãe para refinar a busca')}}"
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 pt-2">
                        <a href="{{ route($cancelRouteName) }}" wire:navigate
                ...
                {{-- Exibe a mensagem de info (nenhum resultado, etc) --}}
                @if($infoMessage)
                    <div class="mt-4 p-3 text-sm {{ Str::contains($infoMessage, 'Nenhum') ? 'bg-yellow-100 dark:bg-yellow-700/30 text-yellow-700 dark:text-yellow-300' : 'bg-blue-100 dark:bg-blue-700/30 text-blue-700 dark:text-blue-300' }} rounded-md">
                        {{ $infoMessage }}
                    </div>
                @endif
                                ...
                                <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                                    <table class="min-w-full divide-y divide-gray-300 dark:divide-neutral-700">
                                        ...
                                        @foreach ($results as $citizen)
                                            <tr wire:key="citizen-result-{{$citizen->id}}">
                                                ...
                                                <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-0">
                                                    <button type="button" wire:click="selectCitizenAndProceed({{ $citizen->id }})"
                                                            ...
                                                            title="{{__('Selecionar este cidadão e prosseguir')}}">
                                                        <span class="icon-[mdi--check-circle-outline] w-4 h-4 mr-1.5"></span>
                ...
                @elseif ($results && $results->isEmpty() && (!empty($search) || !empty($searchMother)))
                    <div class="border-t dark:border-neutral-700 pt-6 mt-6">
                        <p class="text-center text-orange-600 dark:text-orange-400">{{ $infoMessage ?? __('Nenhum cidadão encontrado com os critérios fornecidos.') }}</p>
                    </div>
                @endif
            </div>
        </div>
